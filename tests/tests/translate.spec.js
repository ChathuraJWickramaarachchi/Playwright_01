...
// You can change this to your real URL
const APP_URL = 'https://www.swiftranslator.com';

//    Your 34 test cases (copied from your message)
...
const SELECTORS = {
  inputArea: 'textarea >> nth=0',           // left textarea
  outputArea: 'textarea >> nth=1',          // right textarea
  clearButton: 'button >> text=Clear',      // clear button
  // translateButton: 'button >> text=↔',   // if needed – most cases are auto
};

test.describe('Singlish → Sinhala transliteration tests', () => {
  TEST_CASES.forEach((tc) => {
    test(`TC ${tc.id} - ${tc.input.substring(0, 40)}...`, async ({ page }) => {
      await page.goto(APP_URL);

      // Optional: ensure Singlish mode (if there's a dropdown or toggle)
      // await page.selectOption('select#mode', 'singlish'); // ← adjust if exists

      // Clear previous content if possible
      try {
        await page.locator(SELECTORS.clearButton).click({ timeout: 4000 });
      } catch {
        // ignore if no clear button or timeout
      }

      // Enter text
      const inputLocator = page.locator(SELECTORS.inputArea);
      await inputLocator.waitFor({ state: 'visible', timeout: 10000 });
      await inputLocator.fill(tc.input);

      // Wait for translation (auto-translate)
      await page.waitForTimeout(1800); // adjust time

      // Optional: click translate button if exists
      // await page.locator(SELECTORS.translateButton).click();

      // Get result
      const outputLocator = page.locator(SELECTORS.outputArea);
      const actual = (await outputLocator.inputValue()).trim();
      ...
      if (tc.shouldPass) {
        await expect(actual, `TC ${tc.id} should match expected output`).toBe(tc.expected);
      } else {
        await expect(actual, `TC ${tc.id} should NOT match expected output`).not.toBe(tc.expected);
      }
    });
  });
});